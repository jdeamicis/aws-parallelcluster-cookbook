...
def share_munge_head_node
  ...
  bash 'share_munge_key' do
    ...
    code <<-HEAD_SHARE_MUNGE_KEY
      ...
      mkdir -p #{node['cluster']['shared_dir_head']}/.munge
      chmod 700 #{node['cluster']['shared_dir_head']}/.munge
      # Copy key to shared dir
      cp /etc/munge/munge.key /home/#{node['cluster']['cluster_user']}/.munge/.munge.key
      cp /etc/munge/munge.key #{node['cluster']['shared_dir_head']}/.munge/.munge.key
      chmod 600 #{node['cluster']['shared_dir_head']}/.munge/.munge.key
